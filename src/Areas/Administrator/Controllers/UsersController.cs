...
namespace GladcherryShopping.Areas.Administrator.Controllers
{
    ...
    public class UsersController : Controller
    {
        ...
        public ActionResult Index(string id, int page = 1)
        {
            ...
            {
                query = query.Where(current => current.Id == id);
            }
        ...
        public ActionResult Payments(string id, int page = 1)
        {
            if (id == null)
            {
                return new HttpStatusCodeResult(HttpStatusCode.BadRequest);
            }
            var payments = db.Payments.Where(current => current.UserId == id).Include(p => p.Order).Include(p => p.User);
            PagerViewModels<Payment> PaymentViewModels = new PagerViewModels<Payment>();
            PaymentViewModels.data = payments.Where(current => current.UserId == id).OrderByDescending(current => current.CreateDate).Skip((page - 1) * 10).Take(10).ToList();
        ...
        public ActionResult SearchMobile(int page = 1)
        {
            List<ApplicationUser> users = new List<ApplicationUser>();
            ViewBag.ReturnUrl = "/Administrator/Users/SearchMobile";
            PagerViewModels<ApplicationUser> UserViewModels = new PagerViewModels<ApplicationUser>();
            UserViewModels.CurrentPage = page;
            UserViewModels.data = users;
            UserViewModels.TotalItemCount = users.ToList().Count();
        ...
        public ActionResult MobileResult(string Mobile, string returnUrl, int page = 1)
        {
            if (Mobile == null)
            {
                return new HttpStatusCodeResult(HttpStatusCode.BadRequest);
            }
            var users = db.Users.Where(current => current.Mobile == Mobile).OrderByDescending(current => current.StateId).ThenByDescending(current => current.CityId).ThenByDescending(current => current.LastName).ThenByDescending(current => current.FirstName).Skip((page - 1) * 10).Take(10).ToList();
            PagerViewModels<ApplicationUser> UserViewModels = new PagerViewModels<ApplicationUser>();
            UserViewModels.CurrentPage = page;
            UserViewModels.data = users.OrderByDescending(current => current.LastName).ThenByDescending(current => current.FirstName).Skip((page - 1) * 10).Take(10).ToList();
            UserViewModels.TotalItemCount = users.ToList().Count();
            return View("SearchMobile", UserViewModels);
        }
        ...
        public ActionResult Edit([Bind(Include = "Id,FirstName,LastName,Gender,BirthDate,RegistrationDate,UserScore,ProfileImage,AddressLine,StateId,CityId,Email,EmailConfirmed,PasswordHash,SecurityStamp,PhoneNumber,PhoneNumberConfirmed,TwoFactorEnabled,LockoutEndDateUtc,LockoutEnabled,AccessFailedCount,UserName,IsActive,Credit,VerificationCode,AccessCode,IntroCode,Mobile")] ApplicationUser applicationUser, string birthDateString)
        {
            if (ModelState.IsValid)
            {
                ...
                db.SaveChanges();
                TempData["Success"] = "اطلاعات پروفایل ویرایش شد .";
