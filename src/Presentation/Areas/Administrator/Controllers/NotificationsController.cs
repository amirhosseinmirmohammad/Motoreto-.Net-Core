...
namespace GladcherryShopping.Areas.Administrator.Controllers
{
    ...
    public class NotificationsController : Controller
    {
        ...
        [HttpGet]
        public ActionResult BirthSend(int? id)
        {
            if (id == null)
            {
                return new HttpStatusCodeResult(HttpStatusCode.BadRequest);
            ...
            {
                var role = db.Roles.Where(p => p.Name == "User").FirstOrDefault();
                var query = db.Users.Include(current => current.State).Include(current => current.City).Include(current => current.Notifications).Where(p => p.Roles.Any(a => a.RoleId == role.Id) && p.IsActive == true && /*p.PlayerId != "null" &&*/ !string.IsNullOrEmpty(p.PlayerId) && p.BirthDate != null && p.BirthDate.Value.Year == DateTime.Now.Year && p.BirthDate.Value.Month == DateTime.Now.Month && p.BirthDate.Value.Day == DateTime.Now.Day).ToList();
                if (query.Count() > 0)
                {
                    string[] arr = new string[query.Count()];
                    int i = 0;
                    ...
                    foreach (var item in query.Where(current => current.IsActive == true).ToList())
                    {
                        ...
                        {
                            Code = "dr.sadr_" + notification.Id,
                            Count = 0,
                            MaxCount = query.Count(),
                            IsActived = true,
                            Title = "کد تخفیف یکبار مصرف برای شما ",
                            Amount = "50000"
                        };
                    ...
                    //OneSignal Push Notification
                    //PushNotification(htmlResult, arr);
                    TempData["Success"] = " اطلاعیه شما با موفقیت به  " + query.Count() + " کاربر فعال متولد امروز در سامانه ارسال شد . ";
                }
                else
                {
                    TempData["Error"] = " کاربر متولد امروز در سامانه وجود ندارد .";
                }
            }
            catch (Exception ex)
            {
